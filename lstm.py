...
class LSTMClassifier(nn.Module):
    def __init__(self, word2idx, embedding_dim=300, hidden_dim=256, num_layers=2, num_classes=1, bidirectional=True, seed=8739):
...
class DisasterDatasetLSTM(Dataset):
...
def train_lstm_model(model, train_dataloader, val_dataloader, device, num_epochs=10):
...
def plot_metrics(batch_train_losses, batch_train_accuracies, val_losses, val_accuracies, batch_size, num_epochs):
    ...
    plt.tight_layout()
    plt.savefig('./plots/lstm_disaster_classifier.png')
    plt.close()
...
def train_wrapper(base_model, batch_size, device, num_epochs, seed):
    ...
    train_dataset = DisasterDatasetLSTM(train_tweets, train_targets, word2idx)
    val_dataset = DisasterDatasetLSTM(val_tweets, val_targets, word2idx)
    ...
    model = LSTMClassifier(word2idx=word2idx, seed=seed).to(device)

    batch_train_losses, batch_train_accuracies, val_losses, val_accuracies = train_lstm_model(
...
def evaluate(batch_size, device, model, word2idx, save_path="./datasets/disaster/test_predictions_lstm15.csv"):
    test_tweets, test_ids = load_disaster_test_dataset("./datasets/disaster/test.csv")
    test_dataset = DisasterDatasetLSTM(test_tweets, [0] * len(test_tweets), word2idx)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train LSTM model for disaster tweet classification")
