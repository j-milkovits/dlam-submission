...
class RobertaClassifier(nn.Module):
    def __init__(self, roberta_model="roberta-large", num_classes=1, seed=8739):
...
def train_roberta_model(model, train_dataloader, val_dataloader, device, num_epochs=3):
    optimizer = AdamW(
...
def plot_metrics(
    ...
    for i in range(1, num_epochs + 1):
        plt.axvline(
            x=i * len(batch_train_losses) // num_epochs,
            color="r",
            linestyle="--",
    ...
    for i in range(1, num_epochs + 1):
        plt.axvline(
            x=i * len(batch_train_accuracies) // num_epochs,
            color="r",
            linestyle="--",
            label="Epoch End" if i == 1 else "",
        )
    ...
    plt.plot(
        epochs,
        [
            batch_train_accuracies[i * len(batch_train_accuracies) // num_epochs - 1]
            for i in epochs
        ],
        "bo-",
        label="Train",
    ...
    plt.tight_layout()
    plt.savefig("./plots/roberta_disaster_classifier.png")
    plt.close()
...
def train_wrapper(base_model, batch_size, device, num_epochs, seed):
    ...
    model = RobertaClassifier(roberta_model=base_model, seed=seed).to(device)
    batch_train_losses, batch_train_accuracies, val_losses, val_accuracies = (
        train_roberta_model(model, train_dataloader, val_dataloader, device, num_epochs)
    )
    meta_data = {
    ...
    save_model(model, "./models/roberta_disaster_classifier", meta_data)
    return (
        batch_train_accuracies,
        batch_train_losses,
        model,
...
def evaluate(
    ...
    model,
    tokenizer,
    save_path="./datasets/disaster/test_predictions_roberta.csv",
):
...
def main(base_model="roberta-large", batch_size=32, num_epochs=3, seed=7219):
    batch_acc, batch_loss, model, tokenizer, val_acc, val_loss = train_wrapper(
        base_model, batch_size, device, num_epochs, seed
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train Roberta model for disaster tweet classification"
    )
    parser.add_argument(
        "--base_model",
        type=str,
    ...
    parser.add_argument(
        "--ensemble", type=int, default=0, help="Number of models to train for ensemble"
    )
    ...
    if args.ensemble == 0:
        ...
        for i in range(args.ensemble):
            batch_acc, batch_loss, model, tokenizer, val_acc, val_loss = train_wrapper(
                args.base_model, args.batch_size, device, args.num_epochs, 4868 + i
            )
            evaluate(
                args.batch_size,
                device,
                model,
                tokenizer,
                save_path=f"./datasets/disaster/test_predictions_roberta_{i}.csv",
            )
